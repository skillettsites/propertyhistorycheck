...
import { generateSolicitorHandoverPdf } from "@/lib/pdf/solicitor-handover";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  const pdf = await generateSolicitorHandoverPdf(row.data, liveUrl);
  ...
  return new NextResponse(new Uint8Array(pdf), {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="solicitor-handover-${safeAddress}.pdf"`,
